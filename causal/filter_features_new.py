...
# We now keep BOTH labels side-by-side
OUTCOMES   = ["dep_bin", "BDI2"]        # ← CHG
BDI_THRESH = 14                         # BDI-II ≥14  → “mild or worse” depression
...
# ── HELPERS (unchanged except return-value tweak in load_dep) ──────────
def slope(arr):
...
def load_dep(paths):
    ...
    # Return **both** columns in the expected order
    return dep[["dep_bin", "BDI2"]]      # ← CHG
...
def main():
    ...
    X = X.loc[~X["dep_bin"].isna()]             # ensure at least the binary label
    ...
    feats = X.columns.difference(OUTCOMES)      # ← CHG
